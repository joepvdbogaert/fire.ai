...
def binary_reward(response_time, target):
    return 1 if response_time <= target else 0

def response_time_penalty(response_time, target):
    return -response_time

def linear_lateness_penalty(response_time, target):
    return np.minimum(target - response_time, 0)

def squared_lateness_penalty(response_time, target):
    return -(np.minimum(target - response_time, 0)**2)

def tanh_reward(response_time, target):
